test('Base', function () {
    ...
    equal(jst('comment'), 'Hello!', 'Однострочный комментарий');
    equal(jst('multiline-comment'), 'Hello!', 'Многострочный комментарий');
});
...
test('Attrs', function () {
    equal(jst.attr(), '', 'attr()');
    equal(jst.attr(''), '', "attr('')");
    equal(jst.attr(null), '', "attr(null)");
    equal(jst.attr('abc'), '', "attr('abc')");
    equal(jst.attr('abc', 10), ' abc="10"', "attr('abc', 10)");
    equal(jst.attr('abc', {}), ' abc="[object Object]"', "attr('abc', {})");
    equal(jst.attr('abc', 'abc'), ' abc="abc"', "attr('abc', 'abc')");
    equal(jst.attr({a: 1, b: 2}), ' a="1" b="2"', 'attr({a:1, b:2}');
});
